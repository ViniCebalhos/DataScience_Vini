...
dados_adj <- dados %>% 
  dplyr::mutate(peso_mae = lwt*.453592, # transformando de libras para kg
                ...
                peso_mae_cat = dplyr::case_when(peso_mae <= 49 ~ "<= 49kg",
...
dados_adj %>% group_by(peso_mae_cat) %>% tally()
dados_adj %>% group_by(peso_mae_cat) %>% summarise(min = min(peso_mae),
                                                   max = max(peso_mae))
