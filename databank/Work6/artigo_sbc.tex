...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{Análise de Acidentes em Rodovias Federais Brasileiras}
\fancyhead[R]{Vinícius de Souza Cebalhos}
\fancyfoot[C]{\thepage}
...
Os acidentes de trânsito representam um grave problema de saúde pública no Brasil, resultando em milhares de mortes e feridos anualmente, além de custos econômicos significativos para o sistema de saúde e a sociedade como um todo. Segundo dados do Ministério da Saúde, os acidentes de trânsito são uma das principais causas de morte no país, especialmente entre jovens e adultos em idade produtiva.
...
Os dados utilizados neste estudo foram obtidos a partir de arquivos CSV disponibilizados pela Polícia Rodoviária Federal (PRF), contendo registros detalhados de acidentes de trânsito em rodovias federais brasileiras. Esses dados foram processados e inseridos em um banco de dados PostgreSQL, na tabela \code{especializacao\_vinicius\_acidentes}, para facilitar a realização de consultas e análises.

\subsection{Estrutura da Tabela}

A tabela \code{especializacao\_vinicius\_acidentes} possui os seguintes campos relevantes para a análise:

\begin{itemize}
    \item \code{data\_inversa}: data do acidente (formato YYYY-MM-DD)
    \item \code{horario}: horário do acidente (formato HH:MM)
    \item \code{causa\_acidente}: causa presumida do acidente
    \item \code{uf}: unidade federativa onde ocorreu o acidente
    \item \code{km}: quilômetro da rodovia onde ocorreu o acidente
    \item \code{uop}: unidade operacional responsável pelo trecho
    \item \code{tracado\_via}: tipo de traçado da via (reta, curva, etc.)
\end{itemize}
...
\begin{itemize}
    ...
    \item Existem alguns valores nulos nos campos \code{km}, \code{causa\_acidente} e \code{tracado\_via}
...
Uma consulta SQL foi realizada para contar o número de acidentes por mês, utilizando a função \code{EXTRACT(MONTH FROM data\_inversa)}. Os resultados indicam picos significativos nos seguintes meses:
...
A análise das causas dos acidentes, realizada através de consulta SQL com agrupamento por \code{causa\_acidente}, revelou que as principais causas são:
...
\subsection{Visualizações e Análises Complementares}

Para melhor compreensão e apresentação dos dados, foram geradas as seguintes visualizações e análises:

\begin{itemize}
    \item \textbf{Gráfico de Barras}: representando o número de acidentes por mês, permitindo identificar padrões sazonais (Figura \ref{fig:acidentes_mes})
    \item \textbf{Gráfico de Barras Horizontal}: mostrando as principais causas de acidentes, facilitando a comparação entre diferentes fatores (Figura \ref{fig:causas_acidentes})
    \item \textbf{Gráfico de Linha}: ilustrando a distribuição dos acidentes ao longo das horas do dia, destacando os períodos de maior risco (Figura \ref{fig:acidentes_hora})
\end{itemize}
